...
MY_CPP    = $(HODORFS)/FailureDetector.cpp $(HODORFS)/HodorFS.cpp $(HODORFS)/BufferManager.cpp $(QUERYPARSER)/QueryParser.cpp $(QUERYPARSER)/JoinTable.cpp $(QUERYPARSER)/QueryParserHelper.cpp $(SRC)/HodorDB.cpp $(SRC)/ConsoleReader.cpp $(SRC)/Helper.cpp
...
HodorDB: $(MY_OBJ) $(HYRISE_A)
	g++ -std=c++11 -o HodorDB $(MY_OBJ) $(HYRISE_A) -lpthread
